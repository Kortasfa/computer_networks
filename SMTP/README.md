...
Для тестирования с реальными почтовыми серверами потребуется:
1. Доменное имя или IP-адрес, с которого разрешена отправка
2. Возможно, дополнительная настройка DNS (SPF, DKIM записи)
3. Учитывайте, что письма могут попадать в спам
